...
list_p_T = [0.1, 0.15, 0.2, 0.25]
res_acc=[]
var_acc=[]
for n_accessible in [1]:
    nqubits = n_accessible + n_hidden
    ...
    for p_T in list_p_T:
        ...
        filename = f'results/random_circuit_from_sequence_n{nqubits}_ngates{10*nqubits**2}_p{p_T}.pkl'
...
plt.legend()
plt.savefig(f'results/fourier_analysis/avg_fourier_expressivity_from_sequence_n{nqubits}_ex.png')